...
import {
  PriceFeeding as AnswerUpdatedEvent
} from '../generated/WitnetPriceFeedMTR/WitnetPriceFeed'
...
export function handleAnswerUpdatedMTR(event: AnswerUpdatedEvent): void {
  _storeChainlinkPrice(MTR, event.params.extraFee, event.block.timestamp, event.block.number)
}

export function handleAnswerUpdatedMTRG(event: AnswerUpdatedEvent): void {
  _storeChainlinkPrice(MTRG, event.params.extraFee, event.block.timestamp, event.block.number)
}
